...
class Starter:
    ...
    def __init__(self, key, state, county, tract=None, acsyear=2016):
        ...
        block_group_columns = (
            ...
            + tenure_mover_columns
            + hh_size_columns
            + age_of_head_columns
            + structure_size_columns
            + race_of_head_columns
            + hispanic_head_columns
        )
        tract_columns = vehicle_columns + workers_columns
        self.h_acs = c.block_group_and_tract_query(
            block_group_columns,
            tract_columns,
            state,
            county,
    ...
    def get_household_joint_dist_for_geography(self, ind):
        ...
        h_pums["race_of_head"] = (
            p_pums[p_pums.RELP == 0].groupby("serialno").RAC1P.max()
        )
        h_pums["hispanic_head"] = (
            p_pums[p_pums.RELP == 0].groupby("serialno").HISP.max()
        )
    ...
    def get_person_joint_dist_for_geography(self, ind):
        ...
        if type(puma00) == str:
            ...
            raise ValueError("p_pums not generated")
        ...
        def sex_cat(r):
            return "male" if r.SEX == 1 else "female"
