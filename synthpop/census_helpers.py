...
class Census:
    def __init__(self, key, acsyear=2016):
        ...
        if acsyear >= 2018:
            storage = (
                "https://storage.googleapis.com/synthpop-public"
                "/PUMS2018/pums_2018_acs5/"
            )
        else:
        ...
        self.pums10_population_base_url = self.base_url + "puma10_p_%s_%s.csv"
        self.pums10_household_base_url = self.base_url + "puma10_h_%s_%s.csv"
        self.pums00_population_base_url = self.base_url + "puma00_p_%s_%s.csv"
        self.pums00_household_base_url = self.base_url + "puma00_h_%s_%s.csv"
        self.pums_population_state_base_url = self.base_url + "puma_p_%s.csv"
        self.pums_household_state_base_url = self.base_url + "puma_h_%s.csv"
    ...
    def _scale_and_merge(
        self, df1, tot1, df2, tot2, columns_to_scale, merge_columns, suffixes
    ):
        df = pd.merge(
            df1, df2, left_on=merge_columns, right_on=merge_columns, suffixes=suffixes
        )
    ...
    def block_group_query(
        self, census_columns, state, county, year=2016, tract=None, id=None
    ):
        if id is None:
            id = "*"
        return self._query(
    ...
    def tract_query(self, census_columns, state, county, year=2016, tract=None):
        if tract is None:
            tract = "*"
        return self._query(
            census_columns, state, county, forstr="tract:%s" % tract, year=year
        )
    ...
    def _query(self, census_columns, state, county, forstr, year, tract=None):
        ...
        for census_column_batch in chunks(census_columns, 45):
            ...
            d = c.acs5.get(
                ["NAME"] + census_column_batch,
                geo={"for": forstr, "in": in_str},
                year=year,
            )
            df = pd.DataFrame(d)
    ...
    def block_group_and_tract_query(
        self,
        block_group_columns,
        tract_columns,
        state,
        ...
        df2 = self.tract_query(tract_columns, state, county, tract=tract, year=year)
        df1 = self.block_group_query(
            block_group_columns, state, county, tract=tract, year=year
        )

        df = self._scale_and_merge(
            df1,
            block_group_size_attr,
            df2,
            tract_size_attr,
    ...
    def _get_pums_relationship(self):
        if self.pums_relationship_df is None:
            self.pums_relationship_df = pd.read_csv(
                self.pums_relationship_file_url,
                dtype={
                    "statefp": "object",
                    "countyfp": "object",
                    "tractce": "object",
    ...
    def _get_fips_lookup(self):
        if self.fips_df is None:
            self.fips_df = pd.read_csv(
                self.fips_url,
                dtype={"State ANSI": "object", "County ANSI": "object"},
                index_col=["State", "County Name"],
            )
    ...
    def _read_csv(self, loc, **kargs):
        if loc not in self.pums_cache:
            ...
            pums_df = pums_df.rename(
                columns={"PUMA10": "puma10", "PUMA00": "puma00", "SERIALNO": "serialno"}
            )
    ...
    def download_population_pums(self, state, puma10=None, puma00=None, **kargs):
        ...
        if (puma10 is None) & (puma00 is None):
            return self._read_csv(
                self.pums_population_state_base_url % (state), **kargs
            )
        pums = self._read_csv(
            self.pums10_population_base_url % (state, puma10), **kargs
        )
