...
@pytest.fixture
def freq_wrap(household_freqs, person_freqs, household_constraints, person_constraints):
    return ipu.FrequencyAndConstraints(
        household_freqs, household_constraints, person_freqs, person_constraints
    )
...
def test_fit_quality(
    household_freqs, person_freqs, household_constraints, person_constraints
):
    ...
    npt.assert_allclose(
        ipu._fit_quality(column, weights, constraint), 0.9143, atol=0.0001
    )
    ...
    npt.assert_allclose(
        ipu._fit_quality(column, weights, constraint), 0.3222, atol=0.0003
    )
...
def test_average_fit_quality(household_freqs, freq_wrap):
    weights = np.ones(len(household_freqs), dtype="float")
    npt.assert_allclose(
        ipu._average_fit_quality(freq_wrap, weights), 0.9127, atol=0.0001
    )

    weights = np.array([12.37, 14.61, 8.05, 16.28, 16.91, 8.97, 13.78, 8.97])
    npt.assert_allclose(
        ipu._average_fit_quality(freq_wrap, weights), 0.0954, atol=0.0001
    )
...
def test_update_weights(
    ...
    column = household_freqs[("yes", "blue")]
    column = column.iloc[column.to_numpy().nonzero()[0]]
    constraint = household_constraints[("yes", "blue")]
    weights = pd.Series(np.ones(len(column)), index=column.index)
    ...
    weights = pd.Series(
        [8.05, 9.51, 8.05, 10.59, 11.0, 8.97, 8.97, 8.97], index=range(1, 9)
    ).loc[column.index]
