...
for i in range(73):
    ...
    try:
        ...
        pumageom_file.retrieve(
            "http://www2.census.gov/geo/tiger/PREVGENZ/pu/p500shp/%s" % filename,
...
with zipfile.ZipFile(
    os.path.join(loader.get_path("tract2010_geom"), "Tract_2010Census_DP1.zip"), "r"
) as z:
...
shapefiles = {
    "staging.tracts10": "tract2010_geom/Tract_2010Census_DP1.shp",
}
...
for i in range(73):
    ...
    if os.path.exists(filepath):
        ...
        sql_str = (
            sql_str
            + "select area, perimeter, puma5, name, geom from staging.%s" % subfile_name
        )
        if i < 72:
...
tract10_puma10_rel = pd.read_csv(
    os.path.join(loader.get_path("tract2010_geom"), "tract10_puma10_rel_file.csv"),
    dtype={
        "STATEFP": "object",
        "COUNTYFP": "object",
...
tract10_puma00["TRACTCE"] = tract10_puma00.geoid10.str.slice(
    5,
)
...
tract_puma_xref = tract_puma_xref.rename(
    columns={
        "STATEFP": "statefp",
        "COUNTYFP": "countyfp",
        "TRACTCE": "tractce",
...
tract_puma_xref = tract_puma_xref[
    ["statefp", "countyfp", "tractce", "puma10_id", "puma00_id"]
]
